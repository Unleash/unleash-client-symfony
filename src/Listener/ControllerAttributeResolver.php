...
final class ControllerAttributeResolver implements EventSubscriberInterface
{
    ...
    public function onControllerResolved(ControllerEvent $event): void
    {
        ...
        $item0Unpacked = method_exists($reflectionClass, 'getAttributes') ? $reflectionClass->getAttributes(IsEnabled::class) : [];
        $item1Unpacked = method_exists($reflectionMethod, 'getAttributes') ? $reflectionMethod->getAttributes(IsEnabled::class) : [];
